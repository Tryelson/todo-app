    ...
    <main class="w-[100%] pt-[20px]">
        <div class="w-[calc(100%-40px)] mx-[auto]">
            <header class="flex justify-between items-center">
                <button @click="newTaskModal = true" type="button" class="w-[100%] max-w-[190px] py-[10px] shadow-xl bg-[#3090a9] hover:bg-[#4CCFF1] transition-all duration-200 rounded-lg flex items-center justify-center gap-[10px]">
            ...
            <transition>
                <div v-if="newTaskModal">
                    ...
                    <div class="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] py-[20px] bg-[#181C23] w-[calc(100%-40px)] max-w-[1200px] z-[30] rounded-xl shadow-[0px_0px_20px_black]">
                        <h2 class="px-[20px] mb-[30px] text-[1.875rem]">Criar nova task</h2>
    
                        <form @submit.prevent="($event) => handleCreateNewTask($event)" @invalid="onInvalid" @input="onInput">
                            <fieldset>
                                <div class="w-[calc(100%-40px)] mx-[auto] flex justify-between gap-[20px]">
                                    <div class="flex flex-col gap-[10px] w-[100%] max-w-[260px]">
                                        ...
                                        <input type="text" id="task-name" v-model="task.name" placeholder="Task name" class="bg-[#2C3440] w-[100%] shadow-md py-[5px] px-[7px] rounded-md outline-none border-[1px] border-[#555C65] focus:shadow-[0px_0px_5px_black]" required>
                                    </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            onInput(event) {
                event.target.style.borderColor = '#f5f5f5';
            },
        },
    }
</script>